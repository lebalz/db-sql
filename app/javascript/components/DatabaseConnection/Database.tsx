...
import { Button, Menu, Icon, Segment, Table, Message, Label, Popup, Header } from 'semantic-ui-react';
              ...
              (
                <Fragment>
                  <Header as="h5" attached="top">
                    Result
                    <Popup
                      content={`${_.sumBy(activeConnection.activeDatabase.results, 'time')}s`}
                      trigger={(
                        <Label color="blue">
                          {_.sumBy(activeConnection.activeDatabase.results, 'time').toFixed(2)}s
                        </Label>
                      )}
                    />
                    {
                  ...
                  <Segment attached>
                    {
                      activeConnection.activeDatabase.results.map((result, idx) => {
                        const TimeLabel = (
                          <Popup
                            content={`${result.result ? `${result.result.length} in ` : ''}${result.time}s`}
                            trigger={(
                              <Label
                                color="blue"
                                floating
                              ...
                              >
                                {result.result ? `${result.result.length} in ` : ''}{result.time.toFixed(2)}s
                              </Label>
                            )}
                          />
                        );
